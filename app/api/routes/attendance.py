...
async def get_statistic(
    ...
    if current_teacher.role == 1:
        attend_infors = [
            item for item in attend_infors if item["classId"] == current_teacher.classId
        ]
...
async def search_statistic(
    ...
    attend_infors = await attend_repo.get_statistic_search(
    ...
    summary = {
        ...
        "total_absence_with_permission": sum(
            record["absenceWithPermission"] for record in attend_infors
        )
        or 0,
        ...
        "total_absence_without_permission": sum(
            record["absenceWithoutPermission"] for record in attend_infors
        )
        or 0,
    }
    ...
    if current_teacher.role == 1:
        summary["total_classes"] = (
            attend_infors[0]["className"]
            if attend_infors
            else strings.TEACHER_UNASSIGNED
        )

    return AttendanceStatisList(data=attend_infors, summary=summary)
...
async def attendance(
    ...
    attend_infors = await attend_repo.search_statistic_detail(
        class_id=id, from_date=from_date_parsed, to_date=to_date_parsed
    )
...
async def get_attend_student_detail(
    ...
    attend_infors = await attend_repo.search_attend_student_detail(
        student_id=id, from_date=from_date_parsed, to_date=to_date_parsed
    )
...
async def update_attendance_student(
    ...
    update_row = await attend_repo.update_attendance_student(
        student_update=student_update, attendance_id=attendance_id
    )
