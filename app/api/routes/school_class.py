...
async def get_all_class(
    ...
    for item in all_class:
        data = ClassSelection(
            value=item["id"], label=item["className"], gradeId=item["gradeId"]
        )
...
async def get_all_class(
    ...
    all_class = await class_repo.get_all_class()
    return ClassInfo(data=all_class)
...
async def register_class(
    ...
    missing_fields = [
        field for field, value in class_create.dict().items() if value is ""
    ]
    error_msgs = []
    if missing_fields:
    ...
    class_created = await class_repo.create_class(
        class_name=class_create.class_name, grade_id=class_create.grade_id, quantity=0
    )

    # Assi class for teacher_id
    teacher_update = await teacher_repo.update_class_id(
        teacher_id=class_create.teacher_id, class_id=class_created.id
    )
...
async def update_class(
    ...
    if class_update.teacher_id != teacher["homeroom_class_id"]:
        ...
        await class_repo.update_teacher_is_null(class_id=class_id)
        await teacher_repo.update_class_id(
            teacher_id=class_update.teacher_id, class_id=class_id
        )
    ...
    await class_repo.update_class_by_id(
        class_id=class_id,
