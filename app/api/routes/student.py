...
async def get_student(
    ...
    if current_teacher.role == 1:
        all_student = [
            item for item in all_student if item["classId"] == current_teacher.classId
        ]
...
async def edit_student(
    ...
    student_is_update = await student_repo.student_update(
        id=student_id, current_class_id=current_class_id, object=student_update
    )
