...
async def get_teachers(
    ...
    return TeacherList(data=data_object)
...
async def get_teacher_unassigned(
    ...
    return TeacherUnassignedList(data=data_object)
...
async def register_teacher(
    ...
    missing_fields = [
        field for field, value in teacher_create.dict().items() if value is ""
    ]
    error_msgs = []
    if missing_fields:
    ...
    if (
        await check_teacher_is_taken(teacher_repo, teacher_name=teacher_create.name)
        == True
    ):
...
async def update_teacher(
    ...
    teacher_is_update = await teacher_repo.teacher_update(
