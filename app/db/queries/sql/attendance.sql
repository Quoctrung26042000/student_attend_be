...
FROM
    ...
    INNER JOIN (
        SELECT
            ...
            COUNT(
                CASE
                    WHEN ar.status = 1 THEN 1
                END
            ) AS present_count,
            COUNT(
                CASE
                    WHEN ar.status = 2 THEN 1
                END
            ) AS absent_count,
            COUNT(
                CASE
    ...
    INNER JOIN teacher t ON c.id = t.homeroom_class_id
...
FROM
    ...
    INNER JOIN (
        ...
        FROM
            ...
            JOIN student s ON ar.student_id = s.id
        WHERE
            DATE(ar.create_at) >= :from_date
            AND DATE(ar.create_at) <= :to_date
        GROUP BY
            s.class_id
    ) a ON c.id = a.class_id
    INNER JOIN teacher t ON c.id = t.homeroom_class_id
    INNER JOIN grades g ON c.grade_id = g.id -- WHERE 
...
SELECT
    ...
    COUNT(
        CASE
            WHEN subquery.status = 1 THEN 1
        END
    ) AS "present",
    COUNT(
        CASE
            WHEN subquery.status = 2 THEN 1
        END
    ) AS "absenceWithoutPermission",
    COUNT(
        CASE
            WHEN subquery.status = 3 THEN 1
        END
    ) AS "late",
    COUNT(
        CASE
            WHEN subquery.status = 4 THEN 1
        END
    ) AS "absenceWithPermission"
FROM
    (
        SELECT
            a.id,
...
GROUP BY
    ...
    subquery.grade_id,
    subquery.address,
    subquery.class_name --name: search_attend_student_detail
SELECT
    a.note,
...
FROM
    student
where
    class_id = :class_id --name: approve_all_by_id
UPDATE
    attendance
SET
    status = :status,
