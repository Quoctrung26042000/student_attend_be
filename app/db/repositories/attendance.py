...
class AttendanceRepository(BaseRepository):
    ...
    async def get_statistic_search(self, from_date, to_date):
        attend_row = await queries.get_statistic_search(
            self.connection, from_date=from_date, to_date=to_date
        )
        return attend_row

    async def search_statistic_detail(self, class_id, from_date, to_date):
        attend_row = await queries.search_statistic_detail(
            self.connection, class_id=class_id, from_date=from_date, to_date=to_date
        )
        return attend_row

    async def search_attend_student_detail(self, student_id, from_date, to_date):
        attend_row = await queries.search_attend_student_detail(
            self.connection, student_id=student_id, from_date=from_date, to_date=to_date
        )
        return attend_row

    async def update_attendance_student(self, *, student_update, attendance_id):
        attend_row = await queries.update_attendance_student(
            self.connection,
    ...
    async def approve_all_by_id(self, *, ids, data_object):
        attend_row = await queries.approve_all_by_id(
            self.connection, ids=ids, status=data_object.status, note=data_object.note
        )
        return attend_row

    async def get_students_by_class_id(self, *, class_id):
        ids = await queries.get_students_by_class_id(self.connection, class_id=class_id)
        return ids
