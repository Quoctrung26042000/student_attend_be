...
class StudentRepository(BaseRepository):
    ...
    async def delete_student_by_id(self, *, id: int):
        async with self.connection.transaction():
            ...
            await queries.update_quantity(
                self.connection, variable=-1, class_id=student_row["class_id"]
            )
    ...
    async def create_student(
        ...
        *,
        ...
        async with self.connection.transaction():
            ...
            await queries.insert_new_student_in_attendance(
                self.connection, student_id=student_id
            )

        return student

    async def student_update(self, *, id: int, current_class_id: int, object: dict):
        ...
        async with self.connection.transaction():
            ...
            if current_class_id != student.classId:
                ...
                await queries.update_quantity(
                    self.connection, variable=-1, class_id=current_class_id
                )

                # increasing the number
                await queries.update_quantity(
                    self.connection, variable=1, class_id=student.classId
                )
        return student
