...
class TeacherRepository(BaseRepository):
    async def get_teacher_by_name(self, *, teacher_name: str):
        teacher_row = await queries.get_teacher_by_name(
            self.connection, username=teacher_name
        )
        return teacher_row

    async def get_teacher_by_id(self, *, teacher_id: int):
        teacher_row = await queries.get_teacher_by_id(
            self.connection, teacher_id=teacher_id
        )
        return teacher_row

    async def delete_teacher_by_id(self, *, teacher_id: int):
        teacher_row = await queries.delete_teacher_by_id(
            self.connection, teacher_id=teacher_id
        )
    ...
    async def teacher_update(
        self, *, teacher_id: int, name: str, phone: str, address: Optional[str]
    ):
    ...
    async def update_class_id(self, *, teacher_id: int, class_id: int):
        async with self.connection.transaction():
            teacher_row = await queries.update_class_id(
                self.connection, teacher_id=teacher_id, class_id=class_id
            )
        return teacher_row
    ...
    async def check_phone_is_taken(self, *, phone: str):
        ...
        raise EntityDoesNotExist(
            "teacher with phone {0} does not exist".format(teacher_row)
        )

    async def create_teacher(
        self, *, name: str, phone: str, address: Optional[str]
    ) -> TeacherInDB:
        teacher = TeacherInDB(name=name, phone=phone, address=address)
        async with self.connection.transaction():
